...
```
L7  PRESENTATION    CISO black + threat red (Open Design)
L6  ORCHESTRATION   Hermes sub-context (Kimi K2.6 / DeepSeek V3.5 / local)
L5  DOMAIN MCP      cybersecurity-ai-mcp, owasp-agentic-mcp, security-scanner-ai-mcp
L4  AGENT MEMORY    agentmemory (v0.9.26) — shared mode
L3  KNOWLEDGE GRAPH Cognee subgraph — scope: AI security threats, CVEs, attack surfaces, defensive patterns
L2  VERSIONED HIST  Memoria (v0.4.0) — namespace "asisecurity"
...
```bash
git clone https://github.com/CSOAI-ORG/asisecurity-hive
cd asisecurity-hive
...
`https://asisecurity.ai/.well-known/agent-card.json` — see `agent-card.json`.
...
Bundle with csoai.org suite. CISOs as buyers.
